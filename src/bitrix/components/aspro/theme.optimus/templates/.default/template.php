							...
							<?foreach($option["VALUES"] as $key => $value):?>
								...
								<?if($option["ID"] == 'COLOR_THEME' && $value["VALUE"] == 'CUSTOM'):?>
									<?if(isset($arResult["ITEMS"]['CUSTOM_COLOR_THEME'])):?>
										...
										?>
										<a class="color custom_color_theme colors <?=($bCurrent ? 'active' : '')?>" href="javascript::void(0)" rel="nofollow"  <? /*name="<?=$value["VALUE"]?>"*/ ?> data-option-id="<?=strToLower($option["ID"])?>" data-option-value="<?=strToLower($value["VALUE"])?>">
								...
								<?if($option["ID"] == 'BGCOLOR_THEME' && $value["VALUE"] == 'CUSTOM'):?>
									<?if(isset($arResult["ITEMS"]['CUSTOM_BGCOLOR_THEME'])):?>
										...
										?>
										<a class="color custom_color_theme bg bgcolor <?=($bCurrent ? 'active' : '')?>" href="javascript::void(0)" rel="nofollow"  <? /* name="<?=$value["VALUE"]?>" */ ?> data-option-id="<?=strToLower($option["ID"])?>" data-option-value="<?=strToLower($value["VALUE"])?>">
											<div class="color_block" style="background: <?=$colorCode?>" title="<?=$value["NAME"]?>"></div>
										</a>
