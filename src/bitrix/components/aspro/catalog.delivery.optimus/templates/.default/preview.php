...
<div id="catalog-delivery-preview-<?=$arResult['RAND']?>" class="catalog-delivery-preview">
	...
	<?endif;?>
	<div class="font_xs"><span class="delivery colored_theme_text_with_hover" data-event="jqm" data-param-form_id="delivery" data-name="delivery" data-param-product_id="<?=$arResult['PRODUCT_ID']?>" data-param-quantity="<?=$arResult['PRODUCT_QUANTITY']?>" data-time="<?=time()?>" onclick="$(this).parent().addClass('loadings');$('body').addClass('loading_form');"><?=$arResult['MESSAGES']['PREVIEW_MORE_TITLE']?></span></div>
	<script>
