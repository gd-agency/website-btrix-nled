...
else
{
	...
	$show_licenses = \Bitrix\Main\Config\Option::get('aspro.optimus', 'SHOW_LICENCE', 'N', SITE_ID) === 'Y';
	$show_offer = \Bitrix\Main\Config\Option::get('aspro.optimus', 'SHOW_OFFER', 'N', SITE_ID) === 'Y';

	if( 
		( $show_licenses && !$_REQUEST['licenses_popup_OCB'] ) || 
		( $show_offer && !$_REQUEST['offer_popup_OCB'] )
	){
		die(getJson(GetMessage('FORM_PROCESSING')));
	}
