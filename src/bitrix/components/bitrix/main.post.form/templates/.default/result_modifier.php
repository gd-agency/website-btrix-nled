...
{
	$lheCategory = $arParams["LHE"]["copilotParams"]["category"];
	if (!$arParams["LHE"]["copilotParams"] || !$lheCategory)
	{
		$arParams["COPILOT_AVAILABLE"] = false;
	}
	else
	{
		$arParams["LHE"]["copilotParams"]["engines"] = \Bitrix\AI\Engine::getData('text');

		$lhePrompts = \Bitrix\AI\Prompt\Manager::getCachedTree($lheCategory);

		if ($lhePrompts)
		{
			$arParams["LHE"]["copilotParams"]["prompts"] = $lhePrompts;
		}
		else
		{
			$arParams["COPILOT_AVAILABLE"] = false;
