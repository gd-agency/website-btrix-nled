...
if(WIZARD_INSTALL_DEMO_DATA){
	if(!$iblockID){
		...
		if($_SESSION["WIZARD_OPTIMUS_CATALOG_IBLOCK_ID"] && method_exists('\Bitrix\Iblock\ElementTable', 'getCount'))
		{
			$count = \Bitrix\Iblock\ElementTable::getCount(array(
				'=IBLOCK_ID' => $_SESSION["WIZARD_OPTIMUS_CATALOG_IBLOCK_ID"],
				'=WF_PARENT_ELEMENT_ID' => null
			));
			if ($count > 0)
			{
				$catalogReindex = new CCatalogProductAvailable('', 0, 0);
				$catalogReindex->initStep($count, 0, 0);
				$catalogReindex->setParams(array('IBLOCK_ID' => $_SESSION["WIZARD_OPTIMUS_CATALOG_IBLOCK_ID"]));
				$catalogReindex->run();
				unset($catalogReindex);
			}
		}
		...
		/*bug bitrix not show some sku*/
		$el = new CIBlockElement;

		$rsItems = CIBlockElement::Getlist(array(), array('IBLOCK_ID' => $_SESSION["WIZARD_OPTIMUS_CATALOG_IBLOCK_ID"]), false, false, array('ID', 'ACTIVE'));
