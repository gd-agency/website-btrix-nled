...
<?if($arPost["PARAMS"]):?>
	...
	else
	{
		...
		if(count($arTmpID) > 1)
		{
			...
			$arSelectTmp = array("ID", "NAME", 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PROPERTY_'.$arPost["PARAMS"]["ADD_PICT_PROP"], 'PROPERTY_CML2_ARTICLE');
			...
			while($arItemTmp = $rsItems->Fetch())
			{
				...
				if($arItemTmp['PROPERTY_CML2_ARTICLE_VALUE']){
					$arMainItem['PROPERTIES']['ARTICLE'] = [
						'PROEPRTY_TYPE' => 'S',
						'VALUE' => ": ".$arItemTmp['PROPERTY_CML2_ARTICLE_VALUE']
					];
				}
			}
			if($arTmp)
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			$arItems["ITEMS"][$key] = array(
				...
				"ARTICLE" => ($arPost["PARAMS"]['SHOW_ARTICLE_SKU'] === 'Y' && !$arItem["ARTICLE"] && isset($arItems['MAIN_ITEM']['PROPERTIES']['ARTICLE']) ? $arItems['MAIN_ITEM']['PROPERTIES']['ARTICLE']['VALUE'] : $arItem["ARTICLE"]),
	...
	<script>
		...
		setPriceAction = function(obj, sku, change)
		{
			...
			if(arOptimusOptions['THEME']['SHOW_TOTAL_SUMM'] == 'Y' || arOptimusOptions['THEME']['SHOW_TOTAL_SUMM'] === 'ALWAYS')
			{
		...
		ChangeInfo = function()
		{
			...
			if(-1 < index)
			{
				...
				{
					var article_text = obOffers[index].ARTICLE || '';
					if(
						!article_text 
					...
					if( article_text )
						wrapper.find('.article_block__title').removeClass('hidden')
					else
						wrapper.find('.article_block__title').addClass('hidden')

					if (wrapper.find('.article_block > div').length) {
                        wrapper.find('.article_block > div').text(article_text);
                    } else if( wrapper.find('.article_block__value') ) {
                        wrapper.find('.article_block__value').text(article_text);
                    }
				}
			...
			if (-1 < intNumber && intNumber < $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper').length){
				...
				if (!!RowItems && 0 < RowItems.length){
					...
					for (i = 0; i < RowItems.length; i++){
						...
						// manualy change property title
						if (
							!bHasUnactiveItem
							&& typeof setPropertyTitle === 'function'
							&& obData.style.display === 'none'
							&& !!~RowItems[i].className.indexOf(hideClass) 
						) {
							bHasUnactiveItem = true;
						}
					}

					if (bHasUnactiveItem && activeIndex !== null) {
						const $activeItem = $(RowItems[activeIndex]);

						if ($activeItem.length) {
							setPropertyTitle($activeItem)
						}
					}
		...
		var allProps = <?=CUtil::PhpToJSObject($arItems, false, true)?>
