		...
		foreach($arResult["ORDER_PROPS"] as $block)
		{
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $key => $property)
				{
					...
					?>
					<div class="form-group sale-personal-profile-detail-property-<?=strtolower($property["TYPE"])?> <?=strtolower($property["CODE"])?>">
						...
						<div class="col-md-12">
							...
							elseif ($property["TYPE"] == "TEXT")
							{
								?>
								<input
									class="form-control <?=strtolower($property["CODE"])?>"
									type="text" name="<?=$name?>"
									maxlength="50"
									id="sppd-property-<?=$key?>"
									value="<?=$currentValue?>"/>
							...
							elseif ($property["TYPE"] == "FILE")
							{
								...
								if (count($currentValue) > 0)
								{
									...
									<?
									$profileFiles = Solution::unserialize(htmlspecialchars_decode($currentValue));
