...
<?
	...
	$itemBlockClasses .= ($bBigBlock ? ' col-4' : ' col-2');
?>
						...
						<?if ($arParams["SHOW_SECTION_LIST_PICTURES"]=="Y"):?>
							...
							<td class="image">
								<?if($arItems["PICTURE"]["SRC"]):?>
									...
									<a href="<?=$arItems["SECTION_PAGE_URL"]?>" class="thumb"><img class="img-responsive" src="<?=$img["src"]?>" alt="<?=($arItems["PICTURE"]["ALT"] ? $arItems["PICTURE"]["ALT"] : $arItems["NAME"])?>" title="<?=($arItems["PICTURE"]["TITLE"] ? $arItems["PICTURE"]["TITLE"] : $arItems["NAME"])?>" /></a>
								<?elseif($arItems["~PICTURE"]):?>
									<?$img = CFile::ResizeImageGet($arItems["~PICTURE"], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_EXACT, true );?>
									<a href="<?=$arItems["SECTION_PAGE_URL"]?>" class="thumb"><img class="img-responsive" src="<?=$img["src"]?>" alt="<?=($arItems["PICTURE"]["ALT"] ? $arItems["PICTURE"]["ALT"] : $arItems["NAME"])?>" title="<?=($arItems["PICTURE"]["TITLE"] ? $arItems["PICTURE"]["TITLE"] : $arItems["NAME"])?>" /></a>
								<?else:?>
									<a href="<?=$arItems["SECTION_PAGE_URL"]?>" class="thumb"><img class="img-responsive" src="<?=SITE_TEMPLATE_PATH?>/images/no_photo_medium.png" alt="<?=$arItems["NAME"]?>" title="<?=$arItems["NAME"]?>" height="90" /></a>
								<?endif;?>
							</td>
						<?endif;?>
						<td class="section_info">
							<ul>
								<li class="name<?= $bBigBlock ? ' text-center': ''; ?>">
									...
									<? if( $arItems['ELEMENT_CNT'] && $arParams['SHOW_TOTAL_ELEMENT_COUNT'] === "Y" ): ?>
								...
								<? if ($arParams['SHOW_SECTION_INFO'] === "Y"): ?>
									<? if($arItems["SECTIONS"]): ?>
