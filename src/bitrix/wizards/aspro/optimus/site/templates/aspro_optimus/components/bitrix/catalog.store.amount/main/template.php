...
<?if(count($arResult["STORES"]) > 0):?>
	...
	<div class="stores_block_wrap">
		...
		<?foreach($arResult["STORES"] as $pid => $arProperty):
			if($arParams['SHOW_EMPTY_STORE'] == 'N' && $arProperty['NUM_AMOUNT'] <= 0)
				$empty_count++;?>
			<div class="stores_block <?=(isset($arProperty["IMAGE_ID"]) && !empty($arProperty["IMAGE_ID"]) ? 'w_image' : 'wo_image')?>" style="display: <? echo ($arParams['SHOW_EMPTY_STORE'] == 'N' && $arProperty['NUM_AMOUNT'] <= 0 ? 'none' : ''); ?>;">
