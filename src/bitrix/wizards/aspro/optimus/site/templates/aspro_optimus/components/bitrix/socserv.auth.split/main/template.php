			...
			foreach($arResult["DB_SOCSERV_USER"] as $key => $arUser)
			{
				...
				<tr class="soc-serv-personal">
					...
					<td class="split-item-actions">
						...
						<a class="split-delete-item1 remove" href="?action=delete&user_id=<?=$arUser["ID"]."&".bitrix_sessid_get()?>" onclick="return confirm('<?=GetMessage("SS_PROFILE_DELETE_CONFIRM")?>')" title=<?=GetMessage("SS_DELETE")?>><i></i></a>
						<?endif;?>
					</td>
				</tr>
				<?
