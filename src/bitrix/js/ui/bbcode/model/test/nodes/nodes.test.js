...
describe('ui.bbcode.model/nodes', () => {
	...
	describe('ElementNode', () => {
		it('Create ElementNode with options object', () => {
			...
			assert.ok(node2.isInline() === false);
		...
		it('Should sets inline true automatically for inline elements', () => {
			...
			const italic = scheme.createElement({ name: 'i' });
			assert.ok(italic.isInline() === true);

			const underline = scheme.createElement({ name: 'u' });
			assert.ok(underline.isInline() === true);

			const strike = scheme.createElement({ name: 's' });
			assert.ok(strike.isInline() === true);

			const url = scheme.createElement({ name: 'url' });
			assert.ok(url.isInline() === true);

			const img = scheme.createElement({ name: 'img' });
		...
		it('Should NOT sets inline true automatically for inline elements', () => {
			...
			const code = scheme.createElement({ name: 'code' });
			...
			const list = scheme.createElement({ name: 'list' });
			...
			const code = scheme.createElement({
				...
				children: [
					scheme.createElement({
						name: 'b',
						children: [
							scheme.createText({ content: 'Test' }),
						],
					}),
				],
			});
		...
		it('inline element detection should be case-insensitive', () => {
			...
			const p1Upper = localScheme1.createElement({ name: 'P' });
			const p1Lower = localScheme1.createElement({ name: 'p' });
			const b1Upper = localScheme1.createElement({ name: 'B' });
			const b1Lower = localScheme1.createElement({ name: 'b' });

			const p2Upper = localScheme2.createElement({ name: 'P' });
			const p2Lower = localScheme2.createElement({ name: 'p' });
			const b2Upper = localScheme2.createElement({ name: 'B' });
			const b2Lower = localScheme2.createElement({ name: 'b' });

			const p3Upper = localScheme3.createElement({ name: 'P' });
			const p3Lower = localScheme3.createElement({ name: 'p' });
			const b3Upper = localScheme3.createElement({ name: 'B' });
			const b3Lower = localScheme3.createElement({ name: 'b' });
	...
	describe('Default scheme rules', () => {
		describe('allowedChildren', () => {
			it('b, i, u, s, span should include allowed tags only', () => {
				...
				assert.ok(b.getChildrenCount() === 6);
				assert.ok(b.getChildren().at(0).getName() === 'i');
				assert.ok(b.getChildren().at(1).getName() === 'u');
				assert.ok(b.getChildren().at(2).getName() === 's');
				assert.ok(b.getChildren().at(3).getName() === 'span');
				assert.ok(b.getChildren().at(4).getName() === '#text');
				assert.ok(b.getChildren().at(5).getName() === '#linebreak');
			});
		...
		describe('inline', () => {
			it('b should be inline', () => {
			    const b = scheme.createElement({ name: 'b' });
				assert.ok(b.isInline());
			});

			it('i should be inline', () => {
				const i = scheme.createElement({ name: 'i' });
				assert.ok(i.isInline());
			});

			it('s should be inline', () => {
				const s = scheme.createElement({ name: 's' });
				assert.ok(s.isInline());
			});

			it('p should not be inline', () => {
			    const p = scheme.createElement({ name: 'p' });
				assert.ok(p.isInline() === false);
			});
		});
