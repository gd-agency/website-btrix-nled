...
if($RIGHT >= "R")
{
	...
	foreach($arSites as $key => $arSite)
	{
		...
		$arTabs[] = array(
			...
			"ITEMS" => array(
				"CONFIG" => array(
					"TITLE" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_CONFIG_FLOWLU"),
					"ITEMS" => array(
						"DOMAIN_FLOWLU" => array(
							"TYPE" => "text",
						...
						"TOKEN_FLOWLU" => array(
							...
							"HINT" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_TOKEN_HINT")
						),
					)
				),
				"LINK" => array(
					"TITLE" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_LINK_FLOWLU"),
					"ITEMS" => array(
						...
						"AUTOMATE_SEND_FLOWLU" => array(
							...
							"HINT" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_AUTOMATE_SEND_FLOWLU_HINT")
						),
						"USE_LOG_FLOWLU" => array(
							"TYPE" => "checkbox",
							"VALUE" => Option::get($moduleID, "USE_LOG_FLOWLU", "N", $optionsSiteID),
							"HINT" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_USE_LOG_FLOWLU_HINT")
						),
						"LEAD_NAME_FLOWLU_TITLE" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "LEAD_NAME_FLOWLU_TITLE", Loc::getMessage("ASPRO_OPTIMUS_MODULE_LEAD_NAME_FLOWLU"), $optionsSiteID),
						),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				if(count(explode('.', $_POST["DOMAIN"])) > 2)
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproOptimusCRM::FLOWLU_PATH);
				else
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproOptimusCRM::FLOWLU_SHORT_PATH);

				$arPostFields = array(
					"api_key" => $_POST["TOKEN"]
				);
				echo \Aspro\Functions\CAsproOptimusCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array());
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproOptimus::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'FLOWLU', $_POST["SITE_ID"], false, false);
			}
			else
			{
				$arError = array(
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token)
				{
					if(count(explode('.', $domain)) > 2)
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproOptimusCRM::FLOWLU_PATH);
					else
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproOptimusCRM::FLOWLU_SHORT_PATH);

					$arPostFields = array(
						"api_key" => $token
					);
					$result = json_decode(\Aspro\Functions\CAsproOptimusCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array()), true);
			...
			if($arTab["SITE_ID"])
			{
				...
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK" && !$arOptions["ITEMS"]["ACTIVE_LINK_FLOWLU"]["VALUE"]):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("ASPRO_OPTIMUS_MODULE_INTEGRATION_FLOWLU");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("ASPRO_OPTIMUS_MODULE_".$groupCode."_FLOWLU");?></td></tr>
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_FIELDS_FLOWLU"),
													"TITLE" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_ALL_FIELDS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_RESULTS_FLOWLU"),
													"TITLE" => Loc::getMessage("ASPRO_OPTIMUS_MODULE_ALL_RESULTS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
											...
											<?$siteTabControl->BeginNextTab();?>
												<table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit-table">
													<?foreach($arOption["VALUES"] as $key => $arForm):?>
														...
														$arValueForm = Solution::unserialize(Option::get($moduleID, "FLOWLU_CRM_FIELDS_MATCH_".$key, "", $optionsSiteID));
														?>
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							<?foreach(\Aspro\Functions\CAsproOptimusCRM::$arCrmFileds["FLOWLU"] as $key2 => $text):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							...
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproOptimusCRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
