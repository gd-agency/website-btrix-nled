...
JCSmartFilter.prototype.setUrlSortDisplay = function (url)
{
	$('.sort_btn').each(function(){
		...
		sort = href.match(/.*sort=([^&]*).*/i) ? href.replace(/.*sort=([^&]*).*/i, '$1') : '';
		order = href.match(/.*order=([^&]*).*/i) ? href.replace(/.*order=([^&]*).*/i, '$1') : '';
		display = href.match(/.*display=([^&]*).*/i) ? href.replace(/.*display=([^&]*).*/i, '$1') : '';

		var newUrl = url;
		if(sort.length){
			newUrl = newUrl.replace(/sort=[^&]*/, '') + '&sort=' + sort;
		}
		if(order.length){
			newUrl = newUrl.replace(/order=[^&]*/, '') + '&order=' + order;
		}
		if(display.length){
			newUrl = newUrl.replace(/display=[^&]*/, '') + '&display=' + display;
		}
...
JCSmartFilter.prototype.filterCatalog = function (url, set_disabled)
{
	...
	this.setUrlSortDisplay(decodeURIComponent(url));
	...
	$.ajax({
		...
		success: function(html){
			$('.middle > .container #right_block_ajax_wrapper').html(html);
			// $('.middle > .container #right_block_ajax').html(html);
			jqmEd('ask', arOptimusOptions['FORM']['ASK_FORM_ID'], '.ask_btn');

			/*if(!$('.middle > .container #right_block_ajax .seo_block').length && $('.seo_block').length)
			{
				$('.seo_block').remove();
			}*/
