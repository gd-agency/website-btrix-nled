...
if (CModule::IncludeModule("catalog"))
{
	...
	while ($ar = $rsCatalog->Fetch())
	{
		...
		if(isset($ar["OFFERS_IBLOCK_ID"]) && $ar["OFFERS_IBLOCK_ID"])
		{
			$arSku['IBLOCK_ID'] = $ar["OFFERS_IBLOCK_ID"];
			$arSku['OFFERS_PROPERTY_ID'] = $ar["OFFERS_PROPERTY_ID"];
			$arSku['PRODUCT_IBLOCK_ID'] = $ar["ID"];
		}
	}
}
		...
		case "iblock":
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]))
			{
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					elseif ($arElement['OFFERS'] && \Bitrix\Main\Config\Option::get('aspro.optimus', 'SHOW_FIRST_SKU_PICTURE', 'N') === 'Y') {
						$bFindPicture = false;
					...
					elseif($arSku && $arItem["PARAM2"] == $arSku['IBLOCK_ID'])
					{
						$arTmp = CIBlockElement::GetProperty($arSku['IBLOCK_ID'], $arItem["ITEM_ID"], array("sort" => "asc"), Array("ID"=>$arSku['OFFERS_PROPERTY_ID']))->Fetch();

						if($arTmp['VALUE'])
						{
							$arTmpElement = CIBlockElement::GetList(array(), array('ID' => $arTmp['VALUE'], 'IBLOCK_ID' => $arSku['PRODUCT_IBLOCK_ID']), false, false, array('ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE'))->Fetch();
							if($arTmpElement['PREVIEW_PICTURE'] || $arTmpElement['DETAIL_PICTURE'])
								$arElement["PICTURE"] = CFile::ResizeImageGet(($arTmpElement['PREVIEW_PICTURE'] ? $arTmpElement['PREVIEW_PICTURE'] : $arTmpElement['DETAIL_PICTURE']), array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
						}
					}
				}
			}
