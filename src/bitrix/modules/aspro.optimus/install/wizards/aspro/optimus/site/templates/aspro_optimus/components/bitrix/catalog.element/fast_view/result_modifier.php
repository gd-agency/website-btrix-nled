...
if (empty($arParams['ADD_TO_BASKET_ACTION']) || (!in_array('ADD', (array)$arParams['ADD_TO_BASKET_ACTION']) && !in_array('BUY', (array)$arParams['ADD_TO_BASKET_ACTION'])))
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			if (!empty($arOffer['DISPLAY_PROPERTIES']))
			{
				...
				{
					if ('F' == $arOneProp['PROPERTY_TYPE'] || ($arParams['OFFER_TREE_PROPS'] && in_array($arOneProp['CODE'], (array)$arParams['OFFER_TREE_PROPS'])))
						continue;
			...
			{	
				if(function_exists('CatalogGetPriceTableEx') && (isset($arOffer['PRICE_MATRIX'])) && !$arOffer['PRICE_MATRIX'] && $arPriceTypeID)
				{
					...
					if(count((array)$arOffer['PRICE_MATRIX']['ROWS']) <= 1)
					{
			...
			$arAddToBasketData = COptimus::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'big_btn', $arParams);
