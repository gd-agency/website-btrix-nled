...
describe('scheme/node-schemes/node-scheme', () => {
	...
	describe('NodeScheme.setConverter', () => {
		it('should set the converter property to a function value', () => {
			const converter = (node) => node;
			const scheme = new BBCodeNodeScheme({ name: 'node', convert: converter });
			assert.strictEqual(scheme.getConverter(), converter);
		});

		it('should set the converter property to null', () => {
			const scheme = new BBCodeNodeScheme({ name: 'node', convert: null });
			assert.strictEqual(scheme.getConverter(), null);
		});
	});
