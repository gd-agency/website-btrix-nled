		...
		<div class="bx_filter_section">
			<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
				<div class="bx_filter_parameters_box active title">
					<div class="bx_filter_parameters_box_title"><?=GetMessage('CT_BCSF_FILTER_TITLE');?><i class="svg svg-close close-icons"></i></div>
				</div>
					...
					foreach($arResult["ITEMS"] as $key => $arItem){
					    if(isset($arItem["ASPRO_FILTER_SORT"]) && $arItem["VALUES"]){
							...
							<div class="bx_filter_parameters_box bx_sort_filter <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code="<?=strtolower($arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
								<span data-f="<?=GetMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=GetMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=GetMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=GetMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
					...
					foreach($arResult["ITEMS"] as $key=>$arItem)
					{
						...
						if(isset($arItem["PRICE"])):
							...
							<div class="bx_filter_parameters_box active">
								...
								<div class="bx_filter_parameters_box_title icons_fa" ><?=(count($arParams['PRICE_CODE']) > 1 ? $arItem["NAME"] : GetMessage("PRICE"));?></div>
										...
										<div class="char_name">
											<div class="props_list">
												<?if($arParams["SHOW_HINTS"]){
													...
													if($hint)
														$hint = str_replace(array('line', 'line ', ' line'), '', $hint);
													?>
													<?if(strlen(ltrim($hint))>1){?>
														<div class="hint"><span class="icon"><i>?</i></span><div class="tooltip" style="display: none;"><?=$hint;?></div></div>
													<?}?>
												<?}?>
											</div>
										</div>
					...
					<div class="bx_filter_button_box active">
						<div class="bx_filter_block">
							<div class="bx_filter_parameters_box_container">
								...
								<button class="bx_filter_search_button button small" type="submit" id="set_filter" name="set_filter"  value="<?=GetMessage("CT_BCSF_SET_FILTER")?>"><?=GetMessage("CT_BCSF_SET_FILTER")?></button>
