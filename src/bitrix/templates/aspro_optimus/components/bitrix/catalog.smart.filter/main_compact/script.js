$(document).ready(function(){
	...
	$(document).on('click', "#mobilefilter .bx_filter_parameters_box_title:not(.prices)", function(e){
		...
		if($(this).closest('#mobilefilter').length) {
			$(this).siblings('.bx_filter_block').css('opacity', '').slideToggle(100);
		}
	})
	...
	$(document).on('click', function(e){
		...
		if(window.matchMedia('(min-width:769px)').matches) {
			...
			$(".filter-compact-block .bx_filter_block:not(.limited_block)").slideUp(100);
		}
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		var search_button = $('.bx_filter_block .bx_filter_search_button');
		if(search_button.length && window.matchMedia('(max-width:768px)').matches) {
			var search_text = search_button.text();
			if(search_text.indexOf('(') != -1){
				var start_text = search_text.substr(0, search_text.indexOf('('));
				search_button.text(start_text+' ('+result.ELEMENT_COUNT+')');
			} else {
				search_button.text(search_text+' ('+result.ELEMENT_COUNT+')');
			}
		}
	}
