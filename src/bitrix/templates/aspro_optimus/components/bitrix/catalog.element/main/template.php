...
<div class="item_main_info <?=(!$showCustomOffer ? "noffer" : "");?> <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>" id="<?=$arItemIDs["strMainID"];?>">
				...
				<div class="top_info">
					...
					<?if(strlen($arResult["PREVIEW_TEXT"])):?>
						...
						<?if(strlen($arResult["DETAIL_TEXT"])):?>
							<div class="more_block icons_fa color_link" role="tab" aria-controls="description_panel"><span><?=GetMessage('MORE_TEXT_BOTTOM');?></span></div>
						<?endif;?>
					<?endif;?>
				</div>
			<?}?>
	...
	<?if($arResult['OFFERS']):?>
	    ...
	    <meta itemprop="priceCurrency" content="<?=$currency ? $currency : CSaleLang::GetLangCurrency(SITE_ID)?>" />
		  <meta itemprop="offerCount" content="<?=count((array)$arResult['OFFERS'])?>" />
			<?foreach($arResult['OFFERS'] as $arOffer):?>
...
<div class="tabs_section">
	...
	<ul class="tabs1 main_tabs1 nav-tabs tabs-head" role="tablist">
		...
		<?if($arResult["OFFERS"] && $arParams["TYPE_SKU"]=="N"):?>
			<li class="prices_tab<?=(!($iTab++) ? ' current' : '')?>" id="offer_prices_tab" role="tab" aria-controls="offer_prices_panel">
				<span><?=GetMessage("OFFER_PRICES")?></span>
			</li>
		<?endif;?>
		<?if($arResult["DETAIL_TEXT"] || count((array)$arResult["STOCK"]) || count((array)$arResult["SERVICES"]) || ((count((array)$arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"]) && is_array($arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"])) || count((array)$arResult["SECTION_FULL"]["UF_FILES"])) || ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" id="description_tab" role="tab" aria-controls="description_panel">
				<span><?=GetMessage("DESCRIPTION_TAB")?></span>
			</li>
		<?endif;?>
		<?if($arParams["PROPERTIES_DISPLAY_LOCATION"] == "TAB" && $showProps):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" id="properties_tab" role="tab" aria-controls="properties_panel">
				<span><?=GetMessage("PROPERTIES_TAB")?></span>
			</li>
		<?endif;?>
		<?if($arVideo):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" id="video_tab" role="tab" aria-controls="video_panel">
		...
		<?if($arParams["USE_REVIEW"] == "Y"):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" id="product_reviews_tab" role="tab" aria-controls="product_reviews_panel">
				<span><?=GetMessage("REVIEW_TAB")?></span><span class="count empty"></span>
			</li>
		<?endif;?>
		<?if(($arParams["SHOW_ASK_BLOCK"] == "Y") && (intVal($arParams["ASK_FORM_ID"]))):?>
			<li class="product_ask_tab <?=(!($iTab++) ? ' current' : '')?>" id="ask_tab" role="tab" aria-controls="ask_panel">
				<span><?=GetMessage('ASK_TAB')?></span>
			</li>
		<?endif;?>
		<?if($useStores && ($showCustomOffer || !$arResult["OFFERS"] )):?>
			<li class="stores_tab<?=(!($iTab++) ? ' current' : '')?>" id="stores_tab" role="tab" aria-controls="stores_panel">
				<span><?=GetMessage("STORES_TAB");?></span>
			</li>
		<?endif;?>
		<?if($arParams["SHOW_ADDITIONAL_TAB"] == "Y"):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" id="additional_tab" role="tab" aria-controls="additional_panel">
	...
	<ul class="tabs_content tabs-body">
		...
		<?$iTab = 0;?>
		<?
		$showSkUName = ((in_array('NAME', (array)$arParams['OFFERS_FIELD_CODE'])));
		$showSkUImages = false;
		if(((in_array('PREVIEW_PICTURE', (array)$arParams['OFFERS_FIELD_CODE']) || in_array('DETAIL_PICTURE', (array)$arParams['OFFERS_FIELD_CODE'])))){
		...
		<?if($arResult["OFFERS"] && $arParams["TYPE_SKU"] !== "TYPE_1"):?>
			<li class="prices_tab<?=(!($iTab++) ? ' current' : '')?>" role="tabpanel" aria-labelledby="offer_prices_tab" id="offer_prices_panel">
									...
									<div class="item-actions flexbox flexbox--row">
										...
										<div class="item-buttons item_<?=$arSKU["ID"]?> buy_block counter_block_wr">
											<div class="counter_wrapp list clearfix n-mb small-block">
												<?if($arskuAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && !count((array)$arSKU["OFFERS"]) && $arskuAddToBasketData["ACTION"] == "ADD" && $arSKU["CAN_BUY"]):?>
		...
		<?if($arVideo):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>" role="tabpanel" aria-labelledby="video_tab" id="video_panel">
				...
				<div class="video_block">
					...
					<?else:?>
						<?= str_replace(['frameborder="0"'], '', $arVideo[0]); ?>
					<?endif;?>
				</div>
			</li>
		<?endif;?>
		...
		<?if($arParams["USE_REVIEW"] == "Y"):?>
			<li class="<?=(!($iTab++) ? '' : '')?>" role="tabpanel" aria-labelledby="product_reviews_tab" id="product_reviews_panel"><div class="title-tab-heading js-reviews-tab aspro-bcolor-0099cc visible-xs"><?=($arParams["TAB_REVIEW_NAME"] ? $arParams["TAB_REVIEW_NAME"] : GetMessage("REVIEW_TAB"))?><span class="count empty"></span></div><div id="for_product_reviews_tab" data-reviews-moved></div></li>			
		<?endif;?>
