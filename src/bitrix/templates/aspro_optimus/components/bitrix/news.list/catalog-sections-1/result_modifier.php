...
{
	$arParentSection = COptimusCache::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => COptimusCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N', "CACHE_GROUP" => array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))), array('ID' => $arParams['PARENT_SECTION']), false, array('ID', 'IBLOCK_ID', 'LEFT_MARGIN', 'RIGHT_MARGIN'));
...
if($arResult['SECTIONS']){
	...
	foreach($arResult['SECTIONS'] as $key => $arSection){
		...
		COptimus::getFieldImageData($arResult['SECTIONS'][$key], array('PICTURE'), 'SECTION');
	}
	...
	if($arParams['SHOW_CHILD_SECTIONS'] === 'Y'){
		...
		foreach($arSections as $key => $arSection){
			$arItems = COptimusCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => COptimusCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), "CACHE_GROUP" => array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))), array_merge($arSectionsFilter, array('SECTION_ID' => $arSection['ID']), $arItemsRegionFilter), false, false, array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL'));
